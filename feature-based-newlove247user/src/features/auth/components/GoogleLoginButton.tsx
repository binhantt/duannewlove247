...
export default function GoogleLoginButton() {
  ...
  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setClicked(true);
      try {
        const res = await fetch(`https://www.googleapis.com/oauth2/v2/userinfo?access_token=${tokenResponse.access_token}`);
        const userInfo = await res.json();
        console.log('User Info:', userInfo);
        alert(`Xin chào ${userInfo.name}! Đăng nhập thành công.`);
      } catch (err) {
